const CART_KEY = 'rust_cart'; // Ключ для Rust
...
function renderCart() {
    ...
    if (cart.length === 0) {
        ...
        for (const item of cart) {
            ...
            // Используем title как идентификатор для Rust
            listItem.innerHTML = `
                <img src="images/${item.image}" alt="${item.title}" width="50" height="50">
                <span class="item-title">${item.title}</span> <!-- Убрано качество из скобок -->
                <span class="item-price">${item.price} UAH</span>
                <div class="item-controls">
                    <button class="minus-button" data-title="${item.title}">-</button>
                    <span class="item-quantity">${item.quantity}</span>
                    <button class="plus-button" data-title="${item.title}">+</button>
                    <button class="remove-item" data-title="${item.title}">Видалити</button>
                </div>
            `;
...
document.addEventListener('click', function(event) {
    ...
    // const quality = target.dataset.quality; // Больше не нужен для Rust

    if (target.classList.contains('minus-button') && title) updateItemQuantity(title, -1); // Убрали quality
    else if (target.classList.contains('plus-button') && title) updateItemQuantity(title, 1); // Убрали quality
    else if (target.classList.contains('remove-item') && title) { // Убрали quality
        let cart = localStorage.getItem(CART_KEY);
        cart = cart ? JSON.parse(cart) : [];
        const itemIndex = cart.findIndex(cartItem => cartItem.title === title); // Ищем только по title
        if (itemIndex > -1) {
...
function updateItemQuantity(title, change) { // Убрали qualityIdentifier
    let cart = localStorage.getItem(CART_KEY);
    cart = cart ? JSON.parse(cart) : [];
    const existingItemIndex = cart.findIndex(item => item.title === title); // Ищем только по title
